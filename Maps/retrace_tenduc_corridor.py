# retrace_tenduc_corridor.py (Kris 7/17): the two new roads lacked least-grade routing — my
...
RES=0.03; MS,MN=24.0,52.0; BW,BE=107.0,121.0
TILES=[]
...
KM=RES*111.3
LL={'Tenduc':(111.15,40.35),'Datong':(113.30,40.09),'Khanbaliq':(116.40,39.90),'Xanadu':(116.18,42.36)}
def snap(lo,la):
...
A=route('Tenduc','Datong'); B=route('Datong','Khanbaliq')
full=np.vstack([A,B[1:]])
lenA=np.hypot(np.diff(A[:,0]),np.diff(A[:,1])).sum(); lenB=np.hypot(np.diff(B[:,0]),np.diff(B[:,1])).sum()
frac=lenA/(lenA+lenB)
full_r,_=rs(full,72)
X=route('Khanbaliq','Xanadu')
X_r,_=rs(X,60)
R=json.load(open('Maps/routes_master.json'))
R['legs']['Khanbaliq|Tenduc']=[[round(x,4),round(y,4)] for x,y in full_r[::-1]]
R['legs']['Khanbaliq|Xanadu']=[[round(x,4),round(y,4)] for x,y in X_r]
json.dump(R,open('Maps/routes_master.json','w'))
print('re-traced Khanbaliq|Tenduc (%d pts) and Khanbaliq|Xanadu (%d pts); Datong sits at %.2f from Tenduc'%(len(full_r),len(X_r),frac))
open('/tmp/datong_frac.txt','w').write('%.2f'%frac)
