...
MW,ME,MS,MN=8.0,64.666666666667,52.0,61.0; PPD=147.0
...
ANACHRON=[(37.3,39.4,57.8,59.0),(38.0,38.7,57.1,57.8),(36.4,37.6,56.5,57.1),
 (42.3,44.2,56.5,58.2),   # Gorky res (Volga above Nizhny)
 (46.0,48.2,55.9,56.4),   # Cheboksary res
 (47.3,50.2,52.9,55.7),   # Kuybyshev res - the giant
 (52.0,54.5,55.4,56.3),   # Nizhnekamsk res (Kama)
 (53.4,57.0,56.3,58.2)]   # Votkinsk/Kama res
...
def detail_strip(i,era='1271'):
    ...
    img=img*(1-fm)+col*fm
    if i>=3:
        # SEAM BELT: the south below 52N is classifier everywhere (strip2 committed + the NE fix),
        # and its forest line varies with lon - so the belt IS classifier, blended into old-spec
        # euro going north (full euro by 52.9N). Small lon-ramp at 42E softens the belt notch.
        import climate_biome as cbX
        from scipy.ndimage import gaussian_filter as gfX
        codes,T,P,ocean=cbX.classify_bbox(W,E,MS,MN,DW,DH)
        rng2=np.random.default_rng(7)
        presC=np.zeros((DH,DW),np.float32)
        presC[(codes==cbX.FOREST)|(codes==cbX.TAIGA)]=1.0
        mfs=codes==cbX.FSTEP
        if mfs.any():
            ff=np.clip(cbX.forest_fraction(T,P),0,1); ff=ff*ff*(3-2*ff)
            mn=gfX(rng2.random((DH,DW)).astype(np.float32),2.2)
            mn=(mn-mn.min())/(np.ptp(mn)+1e-9)
            presC[mfs&(mn<ff)]=1.0
        presC=gfX(presC,1.6)
        bl=gfX(rng2.random((DH,DW)).astype(np.float32),4); bl=(bl-bl.min())/(np.ptp(bl)+1e-9)
        presC=presC*0.62*(0.85+0.15*bl)
        presC*=((~water0)&(~snow0)&(~ocean)&(dd>0))
        colX=np.where((codes==cbX.TAIGA)[:,:,None],np.array([48,82,52],np.float32),np.array([78,108,50],np.float32))
        imgC=relief*(1-presC[:,:,None])+colX*presC[:,:,None]
        rampN=np.clip((lat-52.0)/0.9,0,1)
        wB=((1-rampN)*np.clip((lon-41.5)/2.5,0,1))[:,:,None]
        img=img*(1-wB)+imgC*wB
