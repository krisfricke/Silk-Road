# master_gild.py - RECIPE v8 (Kris 7/17): CLASS COLOR DOCTRINE - terrain classes are CANONICAL
# colours (identical wherever they occur), relief carried by shading, near-opaque (0.93), feathered
# only at class boundaries. No more semitransparent blends reading as shadows of the base.
# Baked into MASTER STRIPS 2-4;
...
GOLD = np.array((238,223,163), np.float32)     # canonical dune sea (khwarezm 60.5E,39.6N)
CLAY = np.array((206,196,150), np.float32)     # canonical clay/stony desert (khwarezm Ustyurt 54.5E,44.5N)
SALT = np.array((233,229,213), np.float32)     # canonical salt pan
...
def process_strip(si):
    ...
    for polys, CLR, stipple in (([KYZYLKUM,KARAKUM], GOLD, True), ([USTYURT,ZAUNGUZ,MANGYSH], CLAY, False)):
        ...
        for c in range(3):
            gsh = np.clip(CLR[c]*shade, 0, 255)
            af[:,:,c] = af[:,:,c]*(1-0.93*m) + gsh*0.93*m   # class colour, near-opaque: terrain, not tint
    ...
    af = a.astype(np.float32)
    for c in range(3):
        af[:,:,c] = af[:,:,c]*(1-0.55*rib) + RIPAR[c]*0.55*rib   # tugai is a terrain, not a tint
    a = np.clip(af, 0, 255).astype(np.uint8)
    # dry the salt pans
    for (bw, be, bs, bn) in DRYBOXES:
        ...
        gg = cv2.cvtColor(reg, cv2.COLOR_RGB2GRAY).astype(np.float32)
        lrf = cv2.GaussianBlur(gg,(0,0),20)+1e-3
        shd = np.clip(gg/lrf, 0.75, 1.2)
        mm = cv2.GaussianBlur(cv2.dilate(mk,np.ones((3,3),np.uint8)).astype(np.float32),(0,0),2)[...,None].clip(0,1)
        regf = reg.astype(np.float32)
        for c in range(3):
            regf[:,:,c] = regf[:,:,c]*(1-0.95*mm[:,:,0]) + np.clip(SALT[c]*shd,0,255)*0.95*mm[:,:,0]
        a[y0:y1,x0:x1] = np.clip(regf,0,255).astype(np.uint8)
