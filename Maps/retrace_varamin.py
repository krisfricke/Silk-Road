...
RES=0.03; MS,MN=24.0,52.0; BW,BE=44.0,63.0
TILES=[]
...
# NE lakes are impassable (Van/Urmia sit ABOVE sea level - the DEM alone cannot see them)
...
_lim=_Im.new('L',(GX,GY),0); _ldr=_ID.Draw(_lim)
for _sr in _lk.iterShapes():
    bb=_sr.bbox
    if bb[2]<BW or bb[0]>BE or bb[3]<MS or bb[1]>MN: continue
    _parts=list(_sr.parts)+[len(_sr.points)]
    for _pi in range(len(_parts)-1):
        _ring=[((x-BW)/RES,(MN-y)/RES) for x,y in _sr.points[_parts[_pi]:_parts[_pi+1]]]
        if len(_ring)>=3: _ldr.polygon(_ring,fill=255)
import numpy as _np2
ok&= ~( _np2.array(_lim)>0 )
...
LL={'Varamin':(51.65,35.32),'Nishapur':(58.80,36.20),'Yazd':(54.37,31.90),'Erzurum':(41.28,39.90),'Tabriz':(46.29,38.08),'Konya':(32.50,37.87),'Sivas':(37.02,39.75),'Trebizond':(39.72,41.00),'Acre':(35.07,32.93),'Jerusalem':(35.23,31.78),'Damascus':(36.31,33.51),'Aleppo':(37.15,36.21),'Ayas':(35.79,36.77),'Shazhou':(94.66,40.14),'Kamul':(93.51,42.83),'Suzhou':(98.50,39.77),'Ganzhou':(100.45,38.93),
...
LEGS=[('Tabriz','Varamin'),('Nishapur','Varamin'),('Varamin','Yazd')]
import sys
