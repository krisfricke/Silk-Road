...
import sys
MODE=sys.argv[1] if len(sys.argv)>1 and sys.argv[1] in ('masks','paint') else 'paint'
_which=[int(a) for a in sys.argv[2:] if a.isdigit()] or [int(a) for a in sys.argv[1:] if a.isdigit()] or list(range(n))
for i in _which:
    ...
    _dc='master1271/_dem_%02d.npy'%i
    if os.path.exists(_dc): dd=np.load(_dc).astype(np.float32)
    else: dd=dem(W,E,MS,MN,DW,DH).astype(np.float32); np.save(_dc,dd.astype(np.float16))
    ...
    beshan=((lon>=90.0)&(lon<=97.5)&(lat>=40.3)&(lat<=43.1)).astype(np.float32)
    forest*= (1-cv2.GaussianBlur(beshan,(0,0),30))  # the Beshan/Gashun Gobi is BARREN - the Tian Shan box must not green it (Kris)
    ...
    forest=cv2.remap(forest,mxq0+_wfx,myq0+_wfy,cv2.INTER_LINEAR,borderMode=cv2.BORDER_REPLICATE)  # noise-warp: no straight box edges (Kris caught the right angle NW of Xanadu)
    ...
    _mz='master1271/_masks_%02d.npz'%i
    if MODE=='masks':
        lake,marsh,riv=draw_hydro(W,E,DW,DH,dd)
        np.savez_compressed(_mz,lake=lake,marsh=marsh,riv=riv,forest=forest.astype(np.float16))
        print('masks',i,'saved',flush=True)
        continue
    _z=np.load(_mz)
    lake=_z['lake']; marsh=_z['marsh']; riv=_z['riv']; forest=_z['forest'].astype(np.float32)
    ...
    DUNEP=[[(38.6,28.8),(40.2,27.5),(42.5,27.6),(44.6,28.2),(44.9,28.9),(42.8,29.7),(40.6,29.9),(39.2,29.6)],
           ...
           [(89.5,39.3),(92.5,39.6),(93.5,40.2),(92.0,40.8),(90.0,40.3)],
           [(99.5,39.3),(102.5,38.8),(104.5,37.6),(105.5,38.5),(104.0,40.0),(101.0,40.6),(99.8,40.2)],
           [(85.5,44.7),(88.5,44.9),(89.8,45.7),(87.5,46.2),(85.8,45.6)]]  # Kumtag, Badain Jaran/Tengger, Gurbantunggut (Zaiton)
    ...
    np.save('master1271/_dune_%02d.npy'%i, dune[:, PAD:-PAD].astype(np.float16))
    Image.fromarray(np.clip(img[:,PAD:-PAD],0,255).astype('uint8')).save('master1271/_preripple_%02d.png'%i)
    # DUNE RIPPLE applied separately by apply_ripple.py (tunable without re-detailing)
    ...
    _rl=interp(np.full((1,1),45.0),SEAR)[0,0]   # lighter shelf-blue for the channels themselves
    _halo=(cv2.dilate(riv.astype(np.uint8),np.ones((3,3),np.uint8)).astype(bool))&~riv
    _dark=(img[:,:,0]+img[:,:,1]+img[:,:,2])<390.0
    _hm=(_halo&_dark)[:,:,None].astype(np.float32)*0.38
    img=img*(1-_hm)+np.array([236,225,196],np.float32)[None,None,:]*_hm   # cream halo on dark ground, the house label style
